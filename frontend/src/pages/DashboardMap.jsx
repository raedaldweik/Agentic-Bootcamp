...
/** Dashboard 5, Geography: control is a Sharjah-city phenomenon. The equity gradient, on a map. */
export default function DashboardMap() {
  const [d, setD] = useState(null);
  const [err, setErr] = useState(null);
  const [metric, setMetric] = useState('pct_controlled');
  const [selected, setSelected] = useState(null);
  const [view, setView] = useState('emirates');
  ...
  const outsideSharjah = regions.filter((r) => r.region !== 'Sharjah').reduce((s, r) => s + r.hba1c_overdue, 0);
  const sel = d.facilities.find((f) => f.facility_id === selected);
  const fmt = (v) => metric === 'mean_cost' ? `AED ${Math.round(v).toLocaleString()}`
  ...
  return (
    <div className="h-full p-3 overflow-hidden flex flex-col gap-2.5">
      <div className="flex items-end justify-between shrink-0 px-1">
        <div>
          ...
          <p className="text-[10.5px] mt-1" style={{ color: 'var(--text-dim)' }}>
            Control is concentrated around Sharjah city. Facilities on the East Coast and in the far north carry more open gaps and worse control; distance from the tertiary hospitals and the access gradient follow the same line.
          </p>
        </div>
        <p className="text-[10px]" style={{ color: 'var(--text-faint)' }}>18 facilities · EHS hospitals + primary healthcare centres · gold ring = flagged</p>
      </div>
      ...
      <KpiStrip items={[
        ...
        { icon: 'droplet', tone: 'gold', label: 'HbA1c-overdue patients outside Sharjah', value: outsideSharjah },
      ]} />
          ...
          <Panel title="Facility map: sized by patients, coloured by metric" pad={false}
            right={
              ...
              <div className="seg-track" style={{ padding: 2 }}>
                {[['emirates', 'Northern Emirates'], ['sharjah', 'Greater Sharjah']].map(([k, label]) => (
            ...
            <div className="overflow-y-auto h-full px-3 pb-2">
              {ranked.map((f, i) => (
                <button key={f.facility_id} onClick={() => setSelected(f.facility_id === selected ? null : f.facility_id)}
                  ...
                  style={{ background: selected === f.facility_id ? 'rgba(0,122,115,0.07)' : 'transparent' }}>
                  ...
                  <span className="flex-1 text-[10.5px] font-semibold truncate" style={{ color: 'var(--text-md)' }}>
                    {f.name.replace(' Health Centre', ' HC').replace(' Hospital', ' H')}
            ...
            {sel ? (
              <div className="h-full overflow-y-auto px-2 py-1 grid grid-cols-3 gap-x-3 gap-y-1.5 content-start">
                {[['Patients', sel.patients], ['Region', sel.region], ['Type', sel.type],
                  ...
                  ['Model risk', `${sel.mean_risk_pct}%`], ['Cost / patient', `AED ${sel.mean_cost.toLocaleString()}`],
