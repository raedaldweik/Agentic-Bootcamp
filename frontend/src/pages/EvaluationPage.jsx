 ...
 *   Govern  · the control list, implemented here vs delivered by SAS Viya and SAS RAM
          ...
          <Panel title="Subgroup fairness at the operating threshold">
            <div className="px-1 pb-1 overflow-y-auto" style={{ maxHeight: 420 }}>
              ...
              <p className="text-[9.5px]" style={{ color: 'var(--text-dim)' }}>
                ...
                On SAS Viya these rows are re-run monthly by SAS Model Manager performance monitoring and alert on drift.
              </p>
            </div>
          </Panel>
        </div>
    ...
    <div className="flex flex-col gap-2.5">
      ...
      <div className="grid grid-cols-12 gap-2.5">
        ...
        <div className="col-span-4 flex flex-col gap-2.5">
          <Panel title="What is measured, and what it maps to under SAS RAM">
            <div className="px-1 text-[10px] flex flex-col gap-1.5" style={{ color: 'var(--text-md)' }}>
              {[
                ['Tool-trajectory recall', 'Did the graph call every tool a clinician-reviewer said it must? The same trajectory RAM persists for every query (tool, LLM and retrieval calls), scored in-order-agnostic.'],
                ['Groundedness', 'Clinical claims must carry a guideline citation (doc + page). Under SAS RAM: the retrieval calls and cited passages are persisted with the answer, so a judge can score groundedness on the citation spans.'],
                ...
                ['Latency · tokens · cost', 'Wall-clock per question, tokens from the agent runtime\'s usage metadata, priced at the list rates on the LLM tab.'],
              ].map(([k, v]) => (
                <div key={k}>
          ...
          <Panel title="Release gate in CI">
            <div className="px-1 text-[10px]" style={{ color: 'var(--text-md)' }}>
              <p>The evalset file is ADK-compatible (<code className="text-[9px]">adk eval</code>) and replays against a RAM agent through its API. In CI it runs on every PR:</p>
              <ul className="mt-1 ml-3 list-disc" style={{ color: 'var(--text-dim)' }}>
                ...
                <li>LLM-judge rubric (Claude Opus 5) on tone, completeness and Arabic fidelity → advisory</li>
...
function LlmView() {
  ...
  const perAgent = Object.entries(ac.per_agent_schema_tokens).map(([k, v]) => ({ label: k.replace('_agent', '').replace('basira_', ''), value: v }));
  const saving = ac.monthly_cost_flat_usd - ac.monthly_cost_hierarchical_usd;

  return (
    <div className="flex flex-col gap-2.5">
      <KpiStrip items={[
        { icon: 'coins', tone: 'maroon', label: `Production LLM cost / month · supervisor + specialists (${ac.questions_per_day.toLocaleString()} q/day)`, value: `$${ac.monthly_cost_hierarchical_usd.toLocaleString()}` },
        ...
        { icon: 'coins', tone: 'violet', label: 'Same traffic · Claude Opus 5 everywhere', value: `$${ac.monthly_cost_hierarchical_pro_usd.toLocaleString()}`, trend: `${(ac.monthly_cost_hierarchical_pro_usd / ac.monthly_cost_hierarchical_usd).toFixed(1)}×`, trendDir: 'down' },
        { icon: 'activity', tone: 'sand', label: 'Same traffic · prompt caching on (cache reads at 10%)', value: `$${ac.monthly_cost_hierarchical_cached_usd.toLocaleString()}` },
      ...
      <p className="text-[10px] px-1 -mt-1" style={{ color: 'var(--text-dim)' }}>
        Model plan and cost projection for a customer-owned LLM behind SAS Retrieval Agent Manager, at list prices. The demonstration runs the same agent graph, tools and evalset; the projection is what the production traffic would cost on the chosen models.
      </p>

      <div className="grid grid-cols-12 gap-2.5">
        <div className="col-span-7">
          <Panel title="Production model plan: list prices per 1M tokens (Anthropic price page; any RAM-supported LLM can take the same seat)"
            right={<span className="text-[9.5px]" style={{ color: 'var(--text-faint)' }}>as of {d.prices_as_of}</span>}>
            <div className="px-1">
              <table className="w-full text-[10px]">
                <thead>
                  <tr style={{ color: 'var(--text-faint)' }} className="text-left uppercase tracking-wider">
                    <th className="py-1 font-bold" style={{ width: 118 }}>Model</th><th className="font-bold">Role in Basira</th>
  ...
  return (
    <div className="flex flex-col gap-2.5">
      <KpiStrip items={[
        ...
        { icon: 'activity', tone: 'gold', label: 'Controls delivered by SAS Viya and SAS RAM', value: d.length - impl },
              ...
              {d.filter((x) => x.area === a).map((x) => (
                <div key={x.control} className="flex items-start gap-2 text-[10px] rounded-lg px-2 py-1.5"
                  ...
                  <span className={`badge ${x.status === 'implemented' ? 'badge-green' : 'badge-amber'} shrink-0`} style={{ fontSize: 8, marginTop: 1 }}>
                    {x.status === 'implemented' ? 'implemented' : 'SAS Viya'}
                  </span>
      ...
      <div className="glass-card px-4 py-2.5 text-[10.5px]" style={{ color: 'var(--text-md)' }}>
        <b style={{ color: 'var(--text)' }}>Regulatory frame.</b> The UAE ICT in Health Fields Law (Federal Law 2/2019) and the Personal Data Protection Law (Decree-Law 45/2021) treat health data as sensitive,
        restrict storing or transferring it outside the UAE, and require auditable processing. The design answer is the same in every lane: PHI stays in the UAE, the
...
export default function EvaluationPage() {
  ...
  const sub = useMemo(() => ({
    ...
    governance: 'The control list: implemented in this demonstration, delivered by SAS Viya and SAS RAM in the target architecture, and never permitted.',
  })[view], [view]);
