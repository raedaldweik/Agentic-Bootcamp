...
const md = {
  ...
  table: (p) => (
    <div className="my-2 overflow-x-auto">
      <table className="border-collapse text-[12px] w-full" style={{ border: '1px solid rgba(0,122,115,0.15)' }} {...p} />
    </div>
  ),
  thead: (p) => <thead style={{ background: 'rgba(0,122,115,0.06)' }} {...p} />,
  th: (p) => <th className="px-2 py-1.5 text-left font-semibold" style={{ border: '1px solid rgba(0,122,115,0.15)', color: 'var(--brand)' }} {...p} />,
  td: (p) => <td className="px-2 py-1.5" style={{ border: '1px solid rgba(0,122,115,0.10)' }} {...p} />,
  code: (p) => <code className="px-1 py-0.5 rounded text-[12px]" style={{ background: 'rgba(0,122,115,0.08)', color: 'var(--brand)' }} {...p} />,
        ...
        <div className="flex flex-wrap gap-2">
          {citations.map((c, i) => (
            <button key={i} onClick={() => onOpenSource?.(c)}
              ...
              style={{ color: 'var(--brand-lo)', background: 'rgba(0,122,115,0.06)', border: '1px solid rgba(0,122,115,0.18)' }}>
      ...
      <div className="flex items-center gap-3 px-1 text-[10px]" style={{ color: 'var(--text-faint)' }}>
        {onOpenDetails && (
          <button onClick={() => onOpenDetails(data)} title="Full trace: every agent hop, tool call and token count"
            className="p-1 rounded-md transition-all hover:bg-[rgba(0,122,115,0.10)]" style={{ color: 'var(--text-dim)' }}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
