 ...
 * Browser-native speech; a production build would route audio through the platform's speech services.
 */
  ...
  return (
    <button onClick={toggle} disabled={disabled}
      ...
      className={`relative w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all ${
        listening ? 'bg-red-500/15 text-red-500' : 'hover:bg-[rgba(0,122,115,0.08)]'
      } disabled:opacity-30`}
...
export function SpeakerToggle({ speaker }) {
  return (
    <button
      ...
      className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all hover:bg-[rgba(0,122,115,0.08)]"
