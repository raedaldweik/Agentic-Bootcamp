...
/**
 * "Sign in" button + modal for a RAM deployment that needs an interactive
 * login (health.status === 'signin_required'). Two flows, chosen by the
 * backend (health.signinFlow):
 *   - "device": Keycloak device-code flow (standalone RAM) — show the user
 *     code, open the verification page, poll until approved.
 *   - "code":   Viya SASLogon — open the authorize page in a new tab, the user
 *     pastes the authorization code back here.
 * On success the page reloads so the health pill and agent list refresh.
 */
export default function SignIn({ health }) {
  const { t } = useLanguage();
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                ...
                <p className="text-[11px]" style={{ color: 'var(--text-dim)' }}>
                  {signin.kind === 'code' ? t('signInSso') : t('signInRam')}
                </p>
              </div>
            </div>
