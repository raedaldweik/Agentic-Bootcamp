...
export default function ReportCard({ spec }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden animate-slide-up"
      style={{ border: '1px solid rgba(43,83,120,0.18)', background: 'rgba(255,255,255,0.75)' }}>
      {/* Title bar */}
      <div className="flex items-center gap-2 px-3.5 py-2.5"
        style={{ background: 'rgba(43,83,120,0.06)', borderBottom: '1px solid rgba(43,83,120,0.12)' }}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--sas)" strokeWidth="2" className="shrink-0">
        ...
        {spec.sectionName && (
          <span className="text-[10px] px-1.5 py-0.5 rounded-full font-semibold shrink-0"
            style={{ background: 'rgba(43,83,120,0.10)', color: 'var(--sas-lo)' }}>
            {spec.sectionName}
          </span>
        )}
        <span className="text-[9px] font-bold tracking-wider uppercase px-1.5 py-0.5 rounded shrink-0 ms-auto"
          style={{ background: 'rgba(74,120,160,0.12)', color: '#3A6890' }}>
          SAS VA
        </span>
      </div>
      ...
      {/* Footer */}
      {spec.viewerUrl && (
        <div className="px-3.5 py-2" style={{ borderTop: '1px solid rgba(43,83,120,0.10)' }}>
          <a href={spec.viewerUrl} target="_blank" rel="noreferrer"
            className="text-[11px] font-semibold hover:underline" style={{ color: 'var(--sas)' }}>
            {t('openInVA')}
          </a>
        </div>
      )}
