...
function Section({ title, count, children, defaultOpen = false }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(43,83,120,0.14)' }}>
      <button onClick={() => setOpen(!open)}
        className="w-full flex items-center gap-2 px-4 py-2.5 text-start transition-all hover:bg-[rgba(43,83,120,0.04)]"
        style={{ background: 'rgba(43,83,120,0.05)' }}>
        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--sas)" strokeWidth="2.5"
        ...
        {count != null && (
          <span className="text-[10px] px-1.5 py-0.5 rounded-full font-bold"
            style={{ background: 'rgba(43,83,120,0.10)', color: 'var(--sas)' }}>{count}</span>
        )}
      </button>
...
const Pill = ({ children }) => (
  <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold"
    style={{ background: 'rgba(43,83,120,0.08)', color: 'var(--sas-lo)', border: '1px solid rgba(43,83,120,0.15)' }}>
    {children}
  </span>
);
 ...
 *  and token usage — pulled from RAM's trace for this query.
 */
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-6" onClick={onClose}
      ...
      <div className="glass-card w-full max-w-[760px] max-h-[85vh] flex flex-col animate-slide-up"
        ...
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-[rgba(43,83,120,0.10)]">
          <div className="flex items-center gap-2 min-w-0">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="var(--sas)" strokeWidth="2" className="shrink-0">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-[rgba(43,83,120,0.08)]" style={{ color: 'var(--text-dim)' }}>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          ...
          {/* Question */}
          <div className="rounded-xl px-4 py-3" style={{ background: 'rgba(43,83,120,0.05)', border: '1px solid rgba(43,83,120,0.14)' }}>
          ...
          <Section title={t('toolCallsSection')} count={toolCalls.length} defaultOpen={toolCalls.length > 0}>
            ...
            {toolCalls.map((c, i) => (
              <div key={c.id || i} className="rounded-lg px-3 py-2.5 space-y-2" style={{ border: '1px solid rgba(43,83,120,0.10)' }}>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0"
                    style={{ background: 'rgba(43,83,120,0.12)', color: 'var(--sas)' }}>{i + 1}</span>
                  <span className="text-[12px] font-bold font-mono" style={{ color: 'var(--sas-lo)' }}>{c.toolName || 'tool'}</span>
          ...
          <Section title={t('llmSection')} count={llmCalls.length}>
            ...
            {llmCalls.map((c, i) => (
              <div key={c.id || i} className="rounded-lg px-3 py-2.5 space-y-2" style={{ border: '1px solid rgba(43,83,120,0.10)' }}>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[12px] font-bold font-mono" style={{ color: 'var(--sas-lo)' }}>
                    {c.input?.modelName || c.llmId || 'LLM'}
                  </span>
          ...
          <Section title={t('contextSection')} count={context.length}>
            ...
            {context.map((doc, i) => {
              ...
              return (
                <div key={i} className="rounded-lg px-3 py-2.5" style={{ border: '1px solid rgba(43,83,120,0.10)' }}>
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="text-[11.5px] font-bold truncate" style={{ color: 'var(--sas-lo)' }}>{label}</span>
                    {meta.page != null && <Pill>p. {meta.page}</Pill>}
                    {onOpenSource && (
                      <button onClick={() => onOpenSource(doc)} className="text-[10.5px] font-semibold hover:underline ms-auto shrink-0"
                        style={{ color: 'var(--sas)' }}>{t('open')}</button>
                    )}
                  </div>
