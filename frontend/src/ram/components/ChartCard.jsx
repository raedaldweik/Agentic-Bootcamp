 ...
 *
 * Data source: the `render_chart` MCP tool. The tool does no plotting; this
 * component draws the spec. `yKeys` entries may be plain column names
 * ("pct_controlled") or objects ({ key, label }) — both are accepted.
 */
...
// Normalise a yKeys entry (string or {key,label}) to {key,label}.
function normKey(k) {
  if (k == null) return null;
  if (typeof k === 'string' || typeof k === 'number') return { key: String(k), label: String(k) };
  if (typeof k === 'object' && k.key != null) return { key: String(k.key), label: k.label || k.name || String(k.key) };
  return null;
}
...
export default function ChartCard({ spec }) {
  ...
  const yKeys = useMemo(
    () => (Array.isArray(spec?.yKeys) ? spec.yKeys.map(normKey).filter(Boolean) : []),
    [spec?.yKeys],
  );
  ...
  return (
    <div className="cc-wrap">
      ...
      {(series.length > 1 || type === 'pie' || series[0].label !== series[0].key) && (
        <div className="cc-legend">
          {(type === 'pie' ? labels : series.map((s) => s.label)).map((name, i) => (
...
function Cartesian({ type, labels, series, stacked, xKey, onHover, onLeave }) {
  ...
  const itemsAt = (i) =>
    series.map((s) => ({ name: s.label, color: s.color, value: s.vals[i] }));
