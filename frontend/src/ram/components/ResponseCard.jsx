...
import { extractChartSpecs } from '../chartSpec';
import { extractReportImages } from '../reportSpec';
...
const markdownComponents = {
  ...
  strong: ({node, ...props}) => <strong style={{ color: 'var(--sas)', fontWeight: 700 }} {...props} />,
  em: ({node, ...props}) => <em style={{ color: 'var(--sas-lo)' }} {...props} />,
  ...
  h1: ({node, ...props}) => <h1 className="text-[15px] font-bold my-2" style={{ color: 'var(--sas)' }} {...props} />,
  h2: ({node, ...props}) => <h2 className="text-[14px] font-bold my-2" style={{ color: 'var(--sas)' }} {...props} />,
  h3: ({node, ...props}) => <h3 className="text-[13px] font-semibold my-1.5" style={{ color: 'var(--sas-lo)' }} {...props} />,
  table: ({node, ...props}) => (
    <div className="my-2 overflow-x-auto">
      <table className="border-collapse text-[12px] w-full" style={{ border: '1px solid rgba(43,83,120,0.15)' }} {...props} />
    </div>
  ),
  thead: ({node, ...props}) => <thead style={{ background: 'rgba(43,83,120,0.06)' }} {...props} />,
  th: ({node, ...props}) => <th className="px-2 py-1.5 text-start font-semibold" style={{ border: '1px solid rgba(43,83,120,0.15)', color: 'var(--sas)' }} {...props} />,
  td: ({node, ...props}) => <td className="px-2 py-1.5" style={{ border: '1px solid rgba(43,83,120,0.10)' }} {...props} />,
  // react-markdown v9+ no longer passes `inline`; a fenced block carries a
  // language- class or spans several lines, everything else is inline code.
  code: ({node, className, children, ...props}) => {
    const text = String(children ?? '');
    const block = /language-/.test(className || '') || text.includes('\n');
    return block
      ? <code className={`block p-2 rounded my-1 text-[12px] overflow-x-auto ${className || ''}`} style={{ background: 'rgba(0,0,0,0.04)' }} {...props}>{children}</code>
      : <code className="px-1 py-0.5 rounded text-[12px]" style={{ background: 'rgba(43,83,120,0.08)', color: 'var(--sas)' }} {...props}>{children}</code>;
  },
  pre: ({node, ...props}) => <pre className="my-1" {...props} />,
  blockquote: ({node, ...props}) => <blockquote className="ps-3 my-2 italic" style={{ borderInlineStart: '2px solid var(--sas-lo)', color: 'var(--text-md)' }} {...props} />,
};

/** Renders one normalized agent response: answer, charts, sources, tool calls, usage. */
export default function ResponseCard({ data, onOpenSource, onOpenDetails }) {
  ...
  // Memoize so the specs keep a stable identity across re-renders (e.g. the
  // live-trace polling of the next query).
        ...
        <div className="flex flex-wrap gap-2">
          {data.context.map((doc, i) => (
            <button key={i} onClick={() => onOpenSource?.(doc)}
              ...
              style={{ color: 'var(--sas-lo)', background: 'rgba(43,83,120,0.06)', border: '1px solid rgba(43,83,120,0.18)' }}>
      ...
      <div className="flex items-center gap-3 px-1 text-[10px]" style={{ color: 'var(--text-faint)' }}>
        {onOpenDetails && (
          <button onClick={() => onOpenDetails(data)} title="Details view — tools, LLM calls, retrieval trace"
            className="p-1 rounded-md transition-all hover:bg-[rgba(43,83,120,0.10)]"
