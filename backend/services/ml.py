...
On SAS Viya: the identical scoring call moves to a model published from SAS Model
Manager to MAS/CAS and scored through the SAS Viya MCP server.
"""
...
EVENT_COST_AED = 18500          # average diabetes deterioration episode (admission + follow-up)
...
def similar_patients(patient_id: str, k: int = 6) -> dict:
    ...
    for d, i in zip(dist[0][1:], nbr[0][1:]):
        ...
        out.append({"patient_id": pid, "distance": round(float(d), 2), "age": int(r["age"]),
                    ...
                    "annual_cost_aed": float(r["annual_cost_aed"])})
...
def segment_labels(s: pd.DataFrame) -> pd.Series:
    ...
    feats = pd.DataFrame({"annual_cost_aed": s["annual_cost_aed"], "risk_prob": probs,
...
def segment_summary(df: pd.DataFrame | None = None) -> dict:
    ...
    g = s2.groupby("segment").agg(
        patients=("patient_id", "count"), mean_cost=("annual_cost_aed", "mean"),
        total_cost=("annual_cost_aed", "sum"), mean_risk=("risk_prob", "mean"),
    ...
    return {"segments": [{"segment": k, **{c: (v.item() if hasattr(v, "item") else v)
            ...
            "scatter_sample": s2.sample(min(600, len(s2)), random_state=1)[
                ["risk_prob", "annual_cost_aed", "segment"]].round(4).to_dict("records")}
...
def simulate_policy(intervention: str, horizon_months: int = 12) -> dict:
    ...
    if intervention == "combined":
        ...
        return {"intervention": "combined", "label": "All five programmes combined",
                ...
                "event_cost_avoided_aed": int(sum(p["event_cost_avoided_aed"] for p in parts)),
                "programme_cost_aed": int(sum(p["programme_cost_aed"] for p in parts)),
                "net_benefit_aed": int(sum(p["net_benefit_aed"] for p in parts)),
    ...
    avoided_cost = int(events_avoided * EVENT_COST_AED)
    return {
        ...
        "event_cost_avoided_aed": avoided_cost,
        "programme_cost_aed": programme,
        "net_benefit_aed": avoided_cost - programme,
        "method": ("Counterfactual re-scoring: the eligible cohort is re-scored through the "
                   ...
                   f"event delta is costed at AED {EVENT_COST_AED:,} per avoided deterioration episode."),
    }
