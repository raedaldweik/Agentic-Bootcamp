"""
Basira, direct tool runs for the suggestion chips (resilience layer).
...
async def sc_intensification_gap(persona: str):
    ...
    ev = [
        ...
        {"agent": "risk_agent", "tool": "simulate_policy",
         ...
         "result_summary": f"{sim['expected_events_avoided']:.0f} events avoided, net {_fmt_aed(sim['net_benefit_aed'])}"},
        {"agent": "guideline_agent", "tool": "search_guidelines",
    ...
    net_word = "positive" if sim["net_benefit_aed"] > 0 else "negative"
...
async def sc_national_picture(persona: str):
    ...
    ev = [
        ...
        {"agent": "basira_supervisor", "tool": "render_map", "args_summary": "pct_controlled, flagged facilities ringed",
...
**Geography:** the map shows where control is won and lost across the EHS network. The flagged facilities cluster on the East Coast and in the far north of the Northern Emirates, where distance from the tertiary hospitals and the access gradient follow the same line.
"""
    yield {"type": "final", "answer": answer, "trace": _trace(ev),
           "charts": [
               fac_map,
...
async def sc_cost(persona: str):
    ...
    seg_rows = "\n".join(
        f"| {x['segment']} | {x['patients']:,} | {_fmt_aed(x['mean_cost'])} | {x['mean_risk']*100:.0f}% | {x['mean_gaps']:.1f} |"
        for x in segs)
    answer = f"""**Total annual cost: {_fmt_aed(cc['total_annual_cost_aed'])}**, and it is heavily concentrated: the top 10% of patients drive **{cc['top10pct_share_pct']}%** of all spend.
           ...
           "charts": [
               ...
               {"type": "pie", "title": "Annual spend by segment",
                "data": [{"segment": x["segment"], "spend": round(x["total_cost"] / 1e6, 2)}
                         for x in segs],
                "xKey": "segment", "yKeys": [{"key": "spend", "label": "AED (M)"}]}],
           "citations": [], "actions": [], "usage": None, "model": "direct tool run · live data"}
...
async def sc_policy_sim(persona: str):
    ...
    ev = [
        {"agent": "pophealth_agent", "tool": "simulate_policy",
         ...
         "result_summary": f"{s['expected_events_avoided']:.0f} events avoided, net {_fmt_aed(s['net_benefit_aed'])}"}
        for s in sims
    ] + [{"agent": "risk_agent", "tool": "simulate_policy", "args_summary": "combined, 24 months",
          "result_summary": f"net {_fmt_aed(comb['net_benefit_aed'])}"}]
    async for e in _steps(ev):
        yield e
    rows = "\n".join(
        f"| {s['label'][:48]} | {s['eligible_patients']:,} | {s['relative_risk_reduction_pct']}% "
        f"| {s['expected_events_avoided']:.0f} | {_fmt_aed(s['programme_cost_aed'])} | **{_fmt_aed(s['net_benefit_aed'])}** |"
        for s in sims)
    best = max(sims, key=lambda x: x["net_benefit_aed"]); worst = min(sims, key=lambda x: x["net_benefit_aed"])
...
**All five combined: ≈{comb['expected_events_avoided']:.0f} deterioration events avoided and a net {_fmt_aed(comb['net_benefit_aed'])}** over 24 months (episodes costed at AED {ml.EVENT_COST_AED:,}).
    ...
    yield {"type": "final", "answer": answer, "trace": _trace(ev),
           "charts": [{"type": "bar",
                       ...
                       "data": [{"intervention": s["intervention"].replace("_program", "").replace("_", " "),
                                 "net_aed_m": round(s["net_benefit_aed"] / 1e6, 2)} for s in sims],
                       "xKey": "intervention",
                       "yKeys": [{"key": "net_aed_m", "label": "Net benefit (AED M)"}]}],
           "citations": [], "actions": [], "usage": None, "model": "direct tool run · live data"}
...
On SAS Viya this is one step in SAS Visual Forecasting, or a PROC ESM run the SAS Viya MCP server can execute for the agent, on the same history and seasonality.
"""
