...
# Approximate real-world coordinates (lat, lon) for the EHS network in the Northern Emirates.
FACILITY_COORDS = {
    "F001": (25.3327, 55.4021), "F002": (25.3603, 55.3892), "F003": (25.7880, 55.9517),
    "F004": (25.1276, 56.3370), "F005": (25.5498, 55.5657), "F006": (25.3440, 55.4460),
    "F007": (25.4699, 55.5001), "F008": (25.2870, 55.8813), "F009": (25.3945, 55.4996),
    "F010": (25.8800, 55.9640), "F011": (25.9950, 56.0500), "F012": (25.3938, 55.8530),
    "F013": (25.3060, 56.3560), "F014": (25.4360, 56.3520), "F015": (25.3072, 55.3810),
    "F016": (25.3115, 56.1670), "F017": (25.3400, 56.3475), "F018": (25.5915, 56.2610),
}
...
METRICS = {
    ...
    "mean_cost":      {"label": "Mean annual cost per patient (AED)", "worse_is_high": True, "fmt": "aed"},
...
def facility_metrics() -> list[dict]:
    ...
    for f in fac.itertuples():
        ...
        out.append({
            ...
            "mean_cost": int(g["annual_cost_aed"].mean()),
            "admissions_12mo": int(g["admissions_12mo"].sum()),
            "expat_share_pct": round(float((g["nationality"] != "Emirati").mean()) * 100, 1),
        })
