"""
Basira, the multi-agent system (Google Agent Development Kit).
...
AgentTool so every hop is visible in the event stream and rendered as a trace:
  ...
  pophealth_agent   ★ the population-health MCP server built for this programme,
...
ANTHROPIC_MODELS = ["claude-sonnet-5", "claude-sonnet-4-6", "claude-haiku-4-5"]
GEMINI_MODELS = [
...
def query_bigquery(sql: str) -> dict:
    ...
    basira_hie; tables: patient_summary, patients, conditions, observations, medications,
...
def top_n(sort_by: str, n: int = 10, ascending: bool = False,
    ...
    """Top/bottom N patients by a column (e.g. annual_cost_aed, hba1c_latest, adherence_pdc)."""
...
def render_map(metric: str = "pct_controlled", facility_names_json: str = "",
    ...
    """Emit a colour-coded FACILITY MAP of the EHS network (Northern Emirates) for the UI. metric: pct_controlled |
...
SUPERVISOR_INSTRUCTION = """You are Basira (بصيرة), the population-health AI assistant of Emirates Health Services (EHS), operating on top of the health information exchange for the EHS diabetes registry across the Northern Emirates: 4,000 people living with type 1 or type 2 diabetes, their complications (retinopathy, neuropathy, kidney disease), therapy, adherence and utilisation.
...
You also own render_chart (charts) and render_map (a colour-coded map of the EHS facilities across the Northern Emirates): call them whenever the user asks to see/plot/compare data, or asks about facilities/regions/geography. Keep chart data compact (≤24 rows).
...
def _build_tools_map(model_name: str | None = None):
    ...
    supervisor = LlmAgent(
        name="basira_supervisor", model=llm, generate_content_config=gen_cfg,
        description="Basira population-health supervisor",
...
def _get_runner(model_name: str | None = None):
    ...
    if model not in _runners:
        _runners[model] = Runner(agent=_build_tools_map(model), app_name="basira",
...
async def _stream_once(message: str, session_id: str, persona: str, model: str,
    ...
    sess = await _session_service.get_session(app_name="basira", user_id="demo",
                                              session_id=session_id)
    if sess is None:
        await _session_service.create_session(app_name="basira", user_id="demo",
                                              session_id=session_id)
