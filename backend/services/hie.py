...
disagree. On SAS Viya the same contracts run as CAS / FedSQL queries over the
registry tables; the function contracts stay the same.
"""
...
def top_n(sort_by: str, n: int = 10, ascending: bool = False,
    ...
    cols = select_columns or ["patient_id", "full_name", "age", "nationality", "facility_name",
                              "hba1c_latest", "cv_risk_band", "annual_cost_aed", "care_gap_count"]
...
def cohort_stats(df: pd.DataFrame | None = None) -> dict:
    ...
    if s.empty:
        return {"patients": 0, "with_diabetes": 0, "pct_type1": 0.0, "mean_hba1c": 0.0, "pct_well_controlled": 0.0,
                ...
                "total_annual_cost_aed": 0, "admissions_12mo": 0, "ed_visits_12mo": 0}
    dm = s
    ...
    return {
        ...
        "total_annual_cost_aed": int(s["annual_cost_aed"].sum()),
...
def facility_benchmark(df: pd.DataFrame | None = None) -> list[dict]:
    ...
    g = dm.groupby(["facility_name", "facility_type"]).agg(
        ...
        mean_cost=("annual_cost_aed", "mean"),
    ).round(2).reset_index()
...
def risk_profiles(df: pd.DataFrame | None = None) -> dict:
    ...
    def prof(g):
        ...
        return {"patients": int(len(g)), "mean_risk_pct": round(float(g["risk_prob"].mean()) * 100, 1),
                ...
                "mean_cost_aed": int(g["annual_cost_aed"].mean()),
...
def cost_concentration(df: pd.DataFrame | None = None) -> dict:
    s = (summary() if df is None else df).sort_values("annual_cost_aed", ascending=False).reset_index(drop=True)
    if s.empty:
        return {"total_annual_cost_aed": 0, "top10pct_share_pct": 0.0, "deciles": []}
    total = s["annual_cost_aed"].sum()
    cum = s["annual_cost_aed"].cumsum() / total * 100
    deciles = []
    n = len(s)
    for d in range(1, 11):
    ...
    return {"total_annual_cost_aed": int(total), "top10pct_share_pct": top10_share,
            "deciles": deciles}
...
def quality_measures(df: pd.DataFrame | None = None) -> list[dict]:
    ...
    return [
        m("BASIRA-DM-01", "HbA1c tested in the last 6 months",
          (dm["hba1c_days_since_test"] <= 183).sum(), len(dm), 90),
        m("BASIRA-DM-02", "Glycaemic control, HbA1c <8%",
          (dm["hba1c_latest"] < 8).sum(), len(dm), 70),
        m("BASIRA-DM-03", "Poor control, HbA1c >9% (lower is better)",
          (dm["hba1c_latest"] > 9).sum(), len(dm), 15, higher_is_better=False),
        m("BASIRA-DM-04", "Retinal screening in the last 12 months",
          (dm["retinal_screening_overdue"] == 0).sum(), len(dm), 80),
        m("BASIRA-DM-05", "Diabetic foot exam in the last 12 months",
          (dm["foot_exam_overdue"] == 0).sum(), len(dm), 80),
        m("BASIRA-DM-06", "Urine ACR screening in the last 12 months",
          dm["acr_latest"].notna().sum(), len(dm), 80),
        m("BASIRA-DM-07", "BP controlled <140/90 (diabetes with hypertension)",
          (htn["bp_controlled"] == 1).sum(), len(htn), 65),
        m("BASIRA-DM-08", "SGLT2i / GLP-1 RA in eligible uncontrolled T2DM",
          (t2_elig["on_sglt2_glp1"] == 1).sum(), len(t2_elig), 60),
        m("BASIRA-DM-09", "RAAS inhibitor in CKD or albuminuria",
          (renal["on_raas_inhibitor"] == 1).sum(), len(renal), 80),
        m("BASIRA-DM-10", "Medication adherence ≥80% (PDC)",
