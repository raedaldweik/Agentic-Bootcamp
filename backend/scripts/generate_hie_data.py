"""
Basira, synthetic Health Information Exchange (HIE) generator: the EHS diabetes registry.

Produces a relational, longitudinal, FHIR-shaped synthetic dataset for a UAE-flavoured diabetes population
(Emirates Health Services network, Northern Emirates) (type 2 and type 1), the way a diabetes programme sees it:

    facilities.csv.gz        18 EHS facilities (hospitals + primary healthcare centres)
...
FACILITIES = [
    ...
    # The EHS network across the Northern Emirates: hospitals and primary healthcare centres.
    ("F001", "Al Qassimi Hospital",              "Hospital",       "Sharjah",         1.00),
    ("F002", "Kuwait Hospital Sharjah",          "Hospital",       "Sharjah",         0.90),
    ("F003", "Saqr Hospital",                    "Hospital",       "Ras Al Khaimah",  1.03),
    ("F004", "Fujairah Hospital",                "Hospital",       "Fujairah",        1.05),
    ("F005", "Umm Al Quwain Hospital",           "Hospital",       "Umm Al Quwain",   1.08),
    ("F006", "Wasit Health Centre",              "Primary Care",   "Sharjah",         0.90),
    ("F007", "Al Hamriyah Health Centre",        "Primary Care",   "Sharjah",         0.97),
    ("F008", "Al Dhaid Health Centre",           "Primary Care",   "Sharjah",         1.10),
    ("F009", "Al Hamidiya Health Centre",        "Primary Care",   "Ajman",           0.95),
    ("F010", "Al Rams Health Centre",            "Primary Care",   "Ras Al Khaimah",  1.06),
    ("F011", "Sha'am Health Centre",             "Primary Care",   "Ras Al Khaimah",  1.09),
    ("F012", "Falaj Al Mualla Health Centre",    "Primary Care",   "Umm Al Quwain",   1.12),
    ("F013", "Qidfa Health Centre",              "Primary Care",   "Fujairah",        0.98),
    ("F014", "Al Bidya Health Centre",           "Primary Care",   "Fujairah",        1.01),
    ("F015", "Al Nahda Health Centre",           "Primary Care",   "Sharjah",         0.92),
    ("F016", "Masafi Health Centre",             "Primary Care",   "Fujairah",        0.96),
    ("F017", "Khorfakkan Hospital",              "Hospital",       "Sharjah",         1.06),
    ("F018", "Dibba Hospital",                   "Hospital",       "Fujairah",        1.09),
]

NATIONALITIES = [
    # (name, weight, age_mean, access_factor)  access>1 → worse access/control (equity story)
    ("Emirati",     0.24, 57, 0.92),
    ("Indian",      0.19, 51, 1.06),
    ("Pakistani",   0.10, 52, 1.10),
    ("Egyptian",    0.08, 54, 1.00),
    ("Bangladeshi", 0.08, 47, 1.18),
    ("Filipino",    0.07, 49, 1.04),
    ("Syrian",      0.05, 53, 1.02),
    ("Jordanian",   0.04, 55, 0.98),
    ("Nepali",      0.03, 45, 1.20),
    ("Sudanese",    0.03, 53, 1.05),
    ("Sri Lankan",  0.02, 50, 1.08),
    ("British",     0.02, 58, 0.95),
    ("Other",       0.05, 52, 1.02),
]

NAME_POOLS = {
    "Emirati":     (["Mohammed", "Abdulla", "Hamad", "Khalid", "Saeed", "Rashid", "Sultan", "Ahmed",
                     "Fatima", "Maryam", "Noora", "Aisha", "Shamma", "Hessa", "Sara", "Moza"],
                    ["Al Shamsi", "Al Mansoori", "Al Naqbi", "Al Ketbi", "Al Suwaidi", "Al Zaabi",
                     "Al Nuaimi", "Al Hammadi", "Al Kaabi", "Al Marzooqi", "Al Mazrouei", "Al Dhaheri"]),
    "Syrian":      (["Bassam", "Wael", "Ayman", "Mohannad", "Fadi", "Rima", "Reem", "Lama", "Ghada"],
                    ["Al Halabi", "Kayali", "Haddad", "Barakat", "Nassar", "Shami"]),
...
# How many names at the head of each first-name pool are male; the rest are female.
MALE_FIRSTS = {
    "Emirati": 8, "Syrian": 5, "Indian": 8, "Filipino": 5, "Egyptian": 6, "Bangladeshi": 6, "Nepali": 6,
    "Pakistani": 6, "Sri Lankan": 4, "Sudanese": 4, "Jordanian": 4, "British": 4, "Other": 5,
}
...
# (name, atc, class, annual_cost_aed)
MEDS = {
...
def build_patients() -> pd.DataFrame:
    ...
    for i in range(N_PATIENTS):
        ...
        n_male = MALE_FIRSTS[nat]
        firsts = firsts[:n_male] if gender == "male" else firsts[n_male:]
        ...
        insurance = "Government" if nat == "Emirati" else str(
        ...
        rows.append({
            ...
            "residency_status": "Citizen" if nat == "Emirati" else "Resident",
    ...
    def add(mask, key, base_adherence=0.9):
        ...
        for idx in np.where(mask)[0]:
            ...
            rows.append({
                ...
                "annual_cost_aed": cost if active else 0,
            })
...
def build_summary(pat, meds, obs, enc) -> pd.DataFrame:
    ...
    med_cost = active.groupby("patient_id")["annual_cost_aed"].sum().to_dict()
...
def export_fhir_sample(pat, cond, obs, meds, enc, n_patients=25):
    ...
    nd("Patient", [{
        ...
        "identifier": [{"system": "https://ehs.gov.ae/synthetic-registry/mrn", "value": p.patient_id}],
        "name": [{"text": p.full_name}], "gender": p.gender, "birthDate": p.birth_date,
        "extension": [{"url": "https://basira.demo/fhir/nationality", "valueString": p.nationality}],
...
def main():
    ...
    print(f"deterioration rate (12m label): {s['deterioration_next_12m'].mean()*100:.1f}% | "
          f"total annual cost: AED {s['annual_cost_aed'].sum()/1e6:.1f}M | tiers: {s['registry_risk_tier'].value_counts().to_dict()}")
