"""
Basira, model training pipeline.
...
Trains the real ML models that power the platform (no mocked numbers anywhere):
  ...
  4. visit_forecast      Seasonal linear model over 36 monthly visit counts → 12-month
                         forecast with intervals. (On SAS Viya this becomes SAS Visual Forecasting
                         AI.FORECAST / TimesFM.)
...
SIM_FEATURES = ["age", "bmi", "hba1c_latest", "years_since_diagnosis", "sbp_latest", "egfr_latest",
                "acr_latest", "retinopathy", "neuropathy", "adherence_pdc", "admissions_12mo", "annual_cost_aed"]
...
def train_segments(s: pd.DataFrame, risk_prob: np.ndarray) -> dict:
    feats = pd.DataFrame({
        "annual_cost_aed": s["annual_cost_aed"],
        "risk_prob": risk_prob,
        "age": s["age"],
    ...
    order = prof["annual_cost_aed"].sort_values(ascending=False).index.tolist()
    ...
    mapping = {}
    c = prof["annual_cost_aed"].idxmax(); mapping[c] = "Complex high-cost"; remaining.remove(c)
...
def main():
    ...
    cards = [
        {"model_id": "complication_risk", "name": "Diabetes Deterioration Risk",
         ...
         "phase2": "Gradient boosting in SAS Model Studio (or this XGBoost pipeline in SAS Viya Workbench), registered in SAS Model Manager, published to MAS/CAS and scored through the SAS Viya MCP server's scoring tools."},
        {"model_id": "cohort_segments", "name": "Population Segmentation",
         ...
         "training_data": "4,000 patients", "features": ["annual_cost_aed", "risk_prob", "age", "care_gap_count", "admissions_12mo"],
         "metrics": seg_metrics,
         "intended_use": "Executive cost-of-care narratives and programme design.",
         "limitations": "Segments are descriptive, not causal.",
         "phase2": "PROC KCLUS in CAS (SAS Visual Statistics) over the registry table."},
        {"model_id": "patient_similarity", "name": "Patient Similarity Index",
         ...
         "phase2": "PROC FASTKNN in CAS over the standardised feature space (or SAS Viya embeddings with vector search)."},
        {"model_id": "visit_forecast", "name": "Ambulatory Demand Forecast",
         ...
         "phase2": "SAS Visual Forecasting (or PROC ESM through the SAS Viya MCP server) over the monthly encounter series."},
    ]
