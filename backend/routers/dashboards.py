...
{"tier": "High"} or {"facility": "Al Qassimi Hospital"}): a click on any bar, slice or row
...
def _overview(filters: str):
    ...
    return {
        ...
        "kpis": [
            ...
            {"label": "Annual cost", "value": f"{kpi['total_annual_cost_aed']/1e6:.1f}", "suffix": "M AED"},
        ],
...
def _cost(filters: str):
    ...
    scatter = s.sample(min(500, len(s)), random_state=3)[
        ["hba1c_latest", "annual_cost_aed", "registry_risk_tier"]].round(1)
    return {
        "filter": info,
        "concentration": hie.cost_concentration(s),
        "segments": ml.segment_summary(s)["segments"],
        "by_facility": [{"facility": r["facility_name"],
                         "total_aed_m": round(r["sum"] / 1e6, 2),
                         "mean_aed": int(r["mean"]), "patients": int(r["count"])}
