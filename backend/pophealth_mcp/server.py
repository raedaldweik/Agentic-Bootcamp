"""
population-health-mcp, the population-health MCP server for the SAS x EHS bootcamp.

Why it exists: healthcare MCP servers READ patient records (single-patient lookups,
FHIR resources) and CALL model scoring (the SAS Viya MCP server scores a published
model). But no MCP server lets an agent reason about a POPULATION:
...
Runs over stdio for any MCP client: the Basira agent (ADK McpToolset), SAS Retrieval
Agent Manager (as an MCP tool source), Claude Desktop. Start from backend/:  python -m pophealth_mcp
On SAS Viya: the same tools backed by CAS tables and a published SAS model
...
def get_population_snapshot() -> str:
    ...
    CVD prevalence, open care gaps, utilisation and total annual cost (AED)."""
...
def compute_quality_measure(measure_id: str = "") -> str:
    ...
    Args:
        measure_id: one of BASIRA-DM-01 (HbA1c testing), BASIRA-DM-02 (HbA1c <8%),
            BASIRA-DM-03 (HbA1c >9%, lower is better), BASIRA-DM-04 (retinal screening),
            BASIRA-DM-05 (foot exam), BASIRA-DM-06 (ACR screening), BASIRA-DM-07 (BP control),
            BASIRA-DM-08 (SGLT2i/GLP-1 in eligible T2DM), BASIRA-DM-09 (RAAS in CKD/albuminuria),
            BASIRA-DM-10 (adherence ≥80%).
...
def draft_intervention(patient_ids_json: str, action_type: str, title: str, rationale: str,
    ...
    tool never writes to the EMR. Fills the write-back gap left by read-only healthcare
    healthcare MCP tools, safely.

    Args:
        patient_ids_json: JSON list of patient ids, e.g. '["EHS-100042","EHS-100077"]'.
